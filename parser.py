...
variable = (~keyword + Word(alphas)).addParseAction(
    lambda s, loc, toks: Variable(toks[0]))
...
smpexpression = OneOrMore(atom).addParseAction(
    lambda s, loc, toks: makeApplications(toks))

definition = (variable + Group(ZeroOrMore(variable)) + EQ + expression) \
    .addParseAction(
        lambda s, loc, toks: Definition(toks[0], list(toks[1]), toks[2]))

definitions = Group(delimitedList(definition, delim=","))

letexpression = (LET + definitions + IN + expression) \
